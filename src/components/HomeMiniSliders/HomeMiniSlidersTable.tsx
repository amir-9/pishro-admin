...
import { useHomeMiniSliders, useDeleteHomeMiniSlider } from "@/hooks/api/use-home-mini-sliders";
import type { HomeMiniSlider } from "@/types/api";
import { toast } from "sonner";

const HomeMiniSlidersTable: React.FC = () => {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");
  const { data, isLoading, error } = useHomeMiniSliders({ page, limit: 10, search });
  const deleteHomeMiniSlider = useDeleteHomeMiniSlider();

  const handleDelete = async (id: string) => {
    if (confirm("آیا از حذف این اسلایدر کوچک اطمینان دارید؟")) {
      try {
        await deleteHomeMiniSlider.mutateAsync(id);
        toast.success("اسلایدر کوچک با موفقیت حذف شد");
      } catch (error) {
        toast.error("خطا در حذف اسلایدر کوچک");
      }
    }
  };

  const getRowLabel = (row: number) => {
    return row === 1 ? "ردیف اول" : "ردیف دوم";
  };

  if (error) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4">
        <p className="text-danger">خطا در بارگذاری اسلایدرهای کوچک</p>
      </div>
    );
  }
        ...
        {isLoading ? (
          ...
          <>
            <table className="w-full table-auto">
              ...
              <tbody>
                {data?.items?.map((slider: HomeMiniSlider, index: number) => (
                  <tr key={slider.id}>
                    <td
                    ...
                    >
                      <span
                        className={`inline-block rounded px-2 py-1 text-xs font-medium ${slider.published ? "bg-green bg-opacity-10 text-green" : "bg-red bg-opacity-10 text-red"}`}
                      >
                        {slider.published ? "منتشر شده" : "پیش‌نویس"}
                      </span>
                    </td>
                    ...
                    >
                      <div className="flex gap-2">
                        <Link
                          href={`/home-mini-sliders/edit/${slider.id}`}
            ...
            {data && data.pagination && (
              <div className="mt-4 flex items-center justify-between">
                <p className="text-sm text-dark dark:text-white">
                  نمایش {data.items.length} از {data.pagination.total} اسلایدر
                </p>
...
export default HomeMiniSlidersTable;
