...
const ResumeItemsTable: React.FC = () => {
  ...
  const { data, isLoading, error } = useResumeItems({ page, limit: 10, search });
  const deleteResumeItem = useDeleteResumeItem();
  ...
  if (error) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1">
        <p className="text-danger">خطا در بارگذاری آیتم‌های رزومه</p>
      </div>
    );
  }
                  ...
                  <tr key={item.id}>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-body-sm">{item.color || "-"}</span>
                      </div>
                    </td>
                    ...
                    >
                      <p className="text-dark dark:text-white">{item.order}</p>
                    </td>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Link
                          href={`/resume-items/edit/${item.id}`}
