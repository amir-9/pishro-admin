...
import { useAboutPages, useUpdateAboutPage, useDeleteAboutPage } from "@/hooks/api";

import type { AboutPage } from "@/types/api";
...
const AboutPageTable: React.FC = () => {
  ...
  const { data, isLoading, error } = useAboutPages({ page, limit: 10 });
  const updateAboutPage = useUpdateAboutPage();
  const deleteAboutPage = useDeleteAboutPage();

  const handleTogglePublish = async (id: string, currentStatus: boolean) => {
    if (confirm(`آیا از ${currentStatus ? 'عدم انتشار' : 'انتشار'} این صفحه مطمئن هستید؟`)) {
      try {
        await updateAboutPage.mutateAsync({ id, data: { published: !currentStatus } });
  ...
  const handleDelete = async (id: string) => {
    if (confirm("آیا از حذف این صفحه مطمئن هستید؟ این عملیات قابل بازگشت نیست.")) {
      try {
        await deleteAboutPage.mutateAsync(id);
  ...
  if (error) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4">
        <p className="text-danger">خطا در بارگذاری صفحات درباره</p>
      </div>
    );
  }
    ...
    <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark">
      <div className="border-b border-stroke px-4 py-4 dark:border-dark-3 sm:px-7.5">
        <div className="flex items-center justify-between">
          <h3 className="font-medium text-dark dark:text-white">لیست صفحات درباره</h3>
        </div>
      </div>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            ...
                            className={item.published ? "hover:text-[#FFA70B]" : "hover:text-[#219653]"}
                            title={item.published ? "عدم انتشار" : "انتشار"}
                          ...
                          <button
                            ...
                            title="حذف"
                            disabled={deleteAboutPage.isPending}
                          >
                            <svg
                              className="fill-current"
                              width="20"
