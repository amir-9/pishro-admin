...
const QuizQuestionsTable: React.FC = () => {
  ...
  const handleDelete = async (id: string, question: string) => {
    if (window.confirm(`آیا از حذف سوال "${question}" اطمینان دارید؟`)) {
      try {
  ...
  const getQuestionTypeLabel = (type: string) => {
    const labels: Record<string, string> = {
      MULTIPLE_CHOICE: "چند گزینه‌ای (یک جواب)",
      MULTIPLE_SELECT: "چند گزینه‌ای (چند جواب)",
      TRUE_FALSE: "صحیح/غلط",
  ...
  if (!quizId) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-gray-dark">
        <p className="text-danger">لطفا یک آزمون را انتخاب کنید (quizId در URL الزامی است)</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-gray-dark">
        <p className="text-danger">خطا در بارگذاری سوالات</p>
      </div>
    );
  }
      ...
      <div className="border-b border-stroke px-4 py-4 dark:border-dark-3 sm:px-7.5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              href={`/quiz-questions/create?quizId=${quizId}`}
                    ...
                    <tr key={quizQuestion.id}>
                        ...
                        <div>
                          ...
                          {quizQuestion.explanation && (
                            <p className="text-body text-body-sm line-clamp-1">
                              توضیحات: {quizQuestion.explanation}
                            </p>
                          )}
                        </div>
                      </td>
                      ...
                      >
                        <p className="text-dark dark:text-white">
                          {getQuestionTypeLabel(quizQuestion.questionType)}
                        </p>
                      </td>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <Link
                            href={`/quiz-questions/edit/${quizQuestion.id}`}
                          ...
                          <button
                            onClick={() => handleDelete(quizQuestion.id, quizQuestion.question)}
