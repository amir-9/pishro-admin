...
const BusinessConsultingTable: React.FC = () => {
  ...
  const deleteBusinessConsulting = useDeleteBusinessConsulting();

  const handleTogglePublish = async (id: string, currentStatus: boolean) => {
    if (
      confirm(
        `آیا از ${currentStatus ? "عدم انتشار" : "انتشار"} این صفحه مطمئن هستید؟`,
      )
    ) {
      try {
  ...
  const handleDelete = async (id: string) => {
    if (
      confirm("آیا از حذف این صفحه مطمئن هستید؟ این عملیات قابل بازگشت نیست.")
    ) {
      try {
  ...
  if (error) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4">
        <p className="text-danger">خطا در بارگذاری صفحات مشاوره سرمایه‌گذاری</p>
      </div>
    );
  }
      ...
      <div className="border-b border-stroke px-4 py-4 dark:border-dark-3 sm:px-7.5">
        <div className="flex items-center justify-between">
          <h3 className="font-medium text-dark dark:text-white">
            لیست صفحات مشاوره سرمایه‌گذاری
          </h3>
        </div>
      </div>
                  ...
                  <tr key={item.id}>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Link
                          href={`/business-consulting/edit/${item.id}`}
                        ...
                        <button
                          ...
                          disabled={updateBusinessConsulting.isPending}
                        >
                          <svg
                            className="fill-current"
                            width="20"
                        ...
                        <button
                          ...
                          title="حذف"
                          disabled={deleteBusinessConsulting.isPending}
                        >
                          <svg
                            className="fill-current"
                            width="20"
