...
const BlogCategoriesTable: React.FC = () => {
  ...
  const { data, isLoading, error } = useCategories({ page, limit: 10, search });

  if (error) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4">
        <p className="text-danger">خطا در بارگذاری دسته‌بندی‌های بلاگ</p>
      </div>
    );
  }
                  ...
                  (category: CategoryWithRelations, index: number) => (
                    <tr key={category.id}>
                      ...
                      >
                        ...
                        >
                          {category.published ? "فعال" : "غیرفعال"}
                        </span>
                      </td>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <Link
                            href={`/blog-categories/edit/${category.id}`}
                            className="hover:text-primary"
                            title="ویرایش"
                          >
                            <svg
