...
import { usePageContents } from "@/hooks/api/use-page-content";
  ...
  if (error) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4">
        <p className="text-danger">خطا در بارگذاری پست‌های بلاگ</p>
      </div>
    );
  }
          ...
          <>
                ...
                {data?.data?.items?.map(
                  (blog: PageContent, index: number) => (
                    <tr key={blog.id}>
                      ...
                      >
                        <span
                          className={`inline-flex rounded-full px-3 py-1 text-body-sm font-medium ${blog.published ? "bg-[#219653]/[0.08] text-[#219653]" : "bg-[#D34053]/[0.08] text-[#D34053]"}`}
                        >
            ...
            {data && (
              <div className="mt-4 flex items-center justify-between">
                <p className="text-body text-body-sm">
                  نمایش {data.data.items.length} از {data.data.pagination.total} پست
                </p>
