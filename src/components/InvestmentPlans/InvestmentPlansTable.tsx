...
const InvestmentPlansTable: React.FC = () => {
  ...
  const { data, isLoading, error } = useInvestmentPlans({ page, limit: 10 });
  const updateInvestmentPlans = useUpdateInvestmentPlans();
  const deleteInvestmentPlans = useDeleteInvestmentPlans();
  ...
  if (error) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4">
        <p className="text-danger">خطا در بارگذاری برنامه‌های سرمایه‌گذاری</p>
      </div>
    );
  }
    ...
    <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark">
      <div className="border-b border-stroke px-4 py-4 dark:border-dark-3 sm:px-7.5">
        <div className="flex items-center justify-between">
          <h3 className="font-medium text-dark dark:text-white">لیست برنامه‌های سرمایه‌گذاری</h3>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <>
            <table className="w-full table-auto">
              ...
              <tbody>
                {data?.data?.items?.map(
                  (item: InvestmentPlans, index: number) => (
                    <tr key={item.id}>
                      <td
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <Link
                            href={`/investment-plans/edit/${item.id}`}
                          ...
                          <button
                            ...
                            disabled={updateInvestmentPlans.isPending}
                          >
                            <svg
                              className="fill-current"
                              width="20"
                          ...
                          <button
                            ...
                            title="حذف"
                            disabled={deleteInvestmentPlans.isPending}
                          >
                            <svg
                              className="fill-current"
                              width="20"
            ...
            {data && (
              <div className="mt-4 flex items-center justify-between">
                <p className="text-body text-body-sm">
                  نمایش {data.data.items.length} از {data.data.pagination.total} برنامه
                </p>
