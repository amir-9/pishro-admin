...
const HomeLandingTable: React.FC = () => {
  ...
  const { data, isLoading, error } = useHomeLanding({ page, limit: 10 });
  const updateHomeLanding = useUpdateHomeLanding();
  const deleteHomeLanding = useDeleteHomeLanding();
  ...
  if (error) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4">
        <p className="text-danger">خطا در بارگذاری صفحات لندینگ</p>
      </div>
    );
  }
    ...
    <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark">
      <div className="border-b border-stroke px-4 py-4 dark:border-dark-3 sm:px-7.5">
        <div className="flex items-center justify-between">
          <h3 className="font-medium text-dark dark:text-white">لیست صفحات لندینگ</h3>
        </div>
      </div>
                    ...
                    <tr key={item.id}>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <Link
                            href={`/home-landing/edit/${item.id}`}
                          ...
                          <button
                            ...
                            disabled={updateHomeLanding.isPending}
                          >
                            <svg
                              className="fill-current"
                              width="20"
                          ...
                          <button
                            ...
                            title="حذف"
                            disabled={deleteHomeLanding.isPending}
                          >
                            <svg
                              className="fill-current"
                              width="20"
...
export default HomeLandingTable;
