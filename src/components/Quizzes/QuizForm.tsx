...
const QuizForm: React.FC<QuizFormProps> = ({ quizId, isEdit = false }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Convert empty values to appropriate types for optional/nullable fields
      const submitData = {
        ...formData,
  ...
  const handleChange = (
    ...
    const isOptionalField = name === "courseId" || name === "categoryId";
    setFormData((prev) => ({
      ...prev,
      [name]:
        type === "checkbox"
          ...
          : type === "number"
            ? value === ""
              ...
              : Number(value)
            : value === ""
              ? isOptionalField ? undefined : null
              : value,
    }));
  };
    ...
    <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark">
      ...
      <form onSubmit={handleSubmit} className="p-7">
        ...
        <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
          ...
          <div className="w-full sm:w-1/2">
            <label className="mb-3 block text-body-sm font-medium text-dark dark:text-white">
              حد قبولی (درصد) <span className="text-red">*</span>
            </label>
            <input
              type="number"
              name="passingScore"
          ...
          <div className="flex flex-col gap-3">
            ...
            <label className="flex cursor-pointer items-center gap-2">
              <input
                type="checkbox"
                name="showResults"
                checked={formData.showResults ?? true}
                onChange={handleChange}
                className="rounded border-stroke"
              />
              <span className="text-body-sm font-medium text-dark dark:text-white">
