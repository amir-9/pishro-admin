...
const QuizzesTable: React.FC = () => {
  ...
  const handleDelete = async (id: string, title: string) => {
    if (window.confirm(`آیا از حذف آزمون "${title}" اطمینان دارید؟`)) {
      try {
                ...
                {data?.data?.items?.map(
                  (quiz: QuizWithRelations, index: number) => (
                    <tr key={quiz.id}>
                      ...
                      >
                        <p className="text-dark dark:text-white">
                          {quiz.timeLimit ? `${quiz.timeLimit} دقیقه` : "نامحدود"}
                        </p>
                      </td>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <Link
                            href={`/quiz-questions?quizId=${quiz.id}`}
                          ...
                          >
                            ...
                            >
                              <path
                                d="M10 3.125C8.96447 3.125 8.125 3.96447 8.125 5C8.125 6.03553 8.96447 6.875 10 6.875C11.0355 6.875 11.875 6.03553 11.875 5C11.875 3.96447 11.0355 3.125 10 3.125ZM10 8.125C8.96447 8.125 8.125 8.96447 8.125 10C8.125 11.0355 8.96447 11.875 10 11.875C11.0355 11.875 11.875 11.0355 11.875 10C11.875 8.96447 11.0355 8.125 10 8.125ZM10 13.125C8.96447 13.125 8.125 13.9645 8.125 15C8.125 16.0355 8.96447 16.875 10 16.875C11.0355 16.875 11.875 16.0355 11.875 15C11.875 13.9645 11.0355 13.125 10 13.125Z"
                                fill=""
                              />
                            </svg>
                          </Link>
