  ...
  if (error) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1">
        <p className="text-danger">خطا در بارگذاری اعضای تیم</p>
      </div>
    );
  }
              ...
              <tbody>
                {data?.data?.items?.map((member: TeamMember, index: number) => (
                  <tr key={member.id}>
                    ...
                    >
                      <p className="text-dark dark:text-white">{member.role}</p>
                    </td>
                    ...
                    >
                      ...
                      >
                        {member.published ? "منتشر شده" : "پیش‌نویس"}
                      </span>
                    </td>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Link
                          href={`/team-members/edit/${member.id}`}
