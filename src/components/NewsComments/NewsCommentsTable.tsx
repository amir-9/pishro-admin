...
const NewsCommentsTable: React.FC = () => {
  ...
  const { data, isLoading, error } = useNewsComments({
    page,
    limit: 10,
    search,
  ...
  const handleDelete = async (commentId: string) => {
    if (confirm("آیا از حذف این نظر مطمئن هستید؟ این عملیات قابل بازگشت نیست.")) {
      try {
        await deleteNewsComment.mutateAsync(commentId);
  ...
  if (error) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4">
        <p className="text-danger">خطا در بارگذاری نظرات خبری</p>
      </div>
    );
  }
    ...
    <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark">
      <div className="border-b border-stroke px-4 py-4 dark:border-dark-3 sm:px-7.5">
        <div className="flex items-center justify-between">
          <h3 className="font-medium text-dark dark:text-white">لیست نظرات اخبار</h3>

          <div className="flex gap-3">
            <input
              type="text"
              placeholder="فیلتر شناسه خبر..."
              value={articleIdFilter}
              onChange={(e) => setArticleIdFilter(e.target.value)}
              className="w-40 rounded border border-stroke bg-transparent px-3 py-1.5 outline-none focus:border-primary"
            />
            <input
              type="text"
            ...
            />
            <Link
              href="/news-comments/create"
              className="rounded bg-primary px-4 py-1.5 text-white hover:bg-opacity-90"
            >
              افزودن نظر جدید
            </Link>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            <table className="w-full table-auto">
              ...
              <tbody>
                {data?.data?.items?.map(
                  (comment: NewsCommentWithRelations, index: number) => (
                    <tr key={comment.id}>
                      <td
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            ...
                            title="حذف"
                            disabled={deleteNewsComment.isPending}
                          >
                            <svg
                              className="fill-current"
                              width="20"
