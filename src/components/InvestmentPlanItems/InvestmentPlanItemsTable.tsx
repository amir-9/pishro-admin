...
const InvestmentPlanItemsTable: React.FC = () => {
  ...
  const { data, isLoading, error } = useInvestmentPlanItems({ page, limit: 10, search });
  const deletePlanItem = useDeleteInvestmentPlanItem();
  ...
  if (error) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1">
        <p className="text-danger">خطا در بارگذاری سبدهای سرمایه‌گذاری</p>
      </div>
    );
  }
      ...
      <div className="border-b border-stroke px-4 py-4 dark:border-dark-3 sm:px-7.5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href="/investment-plan-items/create"
        ...
        {isLoading ? (
          ...
          <>
            <table className="w-full table-auto">
              ...
              <tbody>
                {data?.data?.items?.map((plan: InvestmentPlan, index: number) => (
                  <tr key={plan.id}>
                    <td
                    ...
                    >
                      <p className="text-dark dark:text-white">{plan.order}</p>
                    </td>
                    ...
                    >
                      ...
                      >
                        {plan.published ? "منتشر شده" : "پیش‌نویس"}
                      </span>
                    </td>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Link
                          href={`/investment-plan-items/edit/${plan.id}`}
            ...
            {data && (
              <div className="mt-4 flex items-center justify-between">
                <p className="text-body text-body-sm">
                  نمایش {data.data.items.length} از {data.data.pagination.total} سبد
                </p>
...
export default InvestmentPlanItemsTable;
