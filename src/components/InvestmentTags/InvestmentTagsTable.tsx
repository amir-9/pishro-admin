...
const InvestmentTagsTable: React.FC = () => {
  ...
  const { data, isLoading, error } = useInvestmentTags({ page, limit: 10, search });
  const deleteTag = useDeleteInvestmentTag();
  ...
  if (error) {
    return (
      <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1">
        <p className="text-danger">خطا در بارگذاری تگ‌های سرمایه‌گذاری</p>
      </div>
    );
  }
                  ...
                  <tr key={tag.id}>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-body-sm">{tag.color || "-"}</span>
                      </div>
                    </td>
                    ...
                    >
                      <p className="text-dark dark:text-white">{tag.order}</p>
                    </td>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Link
                          href={`/investment-tags/edit/${tag.id}`}
...
export default InvestmentTagsTable;
