...
import { usePageContents, useDeletePageContent } from "@/hooks/api/use-page-content";
...
const PageContentsTable: React.FC = () => {
  ...
  const deletePageContent = useDeletePageContent();

  const handleDelete = async (id: string, title: string) => {
    if (window.confirm(`آیا از حذف محتوا "${title || 'بدون عنوان'}" اطمینان دارید؟`)) {
      try {
        await deletePageContent.mutateAsync(id);
        ...
        {isLoading ? (
          ...
          <>
            <table className="w-full table-auto">
              ...
              <tbody>
                {data?.data?.items?.map(
                  (content: PageContent, index: number) => (
                    <tr key={content.id}>
                      <td
                      ...
                      >
                        <span className="inline-flex rounded-full px-3 py-1 text-body-sm font-medium bg-[#3C50E0]/[0.08] text-[#3C50E0]">
                          {content.type}
                        </span>
                      </td>
                      ...
                      >
                        <p className="text-dark dark:text-white">
                          {format(new Date(content.updatedAt), "yyyy/MM/dd")}
                        </p>
                      </td>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            ...
                            title="حذف"
                            disabled={deletePageContent.isPending}
                          >
                            <svg
                              className="fill-current"
                              width="20"
            ...
            {data && (
              <div className="mt-4 flex items-center justify-between">
                <p className="text-body text-body-sm">
                  نمایش {data.data.items.length} از {data.data.pagination.total} محتوا
                </p>
