...
import { useNewsletterSubscribers, useDeleteNewsletterSubscriber } from "@/hooks/api/use-newsletter-subscribers";
...
const NewsletterSubscribersTable: React.FC = () => {
  ...
  const { data, isLoading, error } = useNewsletterSubscribers({ page, limit: 10, search });
  const deleteSubscriber = useDeleteNewsletterSubscriber();

  const handleDelete = async (id: string, phone: string) => {
    if (window.confirm(`آیا از حذف مشترک "${phone}" اطمینان دارید؟`)) {
      try {
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="inline-flex items-center justify-center rounded bg-success px-4 py-2 text-center font-medium text-white hover:bg-opacity-90"
              >
                <svg
                ...
                >
                  <path d="M20 2H4C2.9 2 2.01 2.9 2.01 4L2 22L6 18H20C21.1 18 22 17.1 22 16V4C22 2.9 21.1 2 20 2ZM9 11H7V9H9V11ZM13 11H11V9H13V11ZM17 11H15V9H17V11Z" fill=""/>
                </svg>
                ارسال پیامک گروهی
              </button>

              <Link
                href="/newsletter-subscribers/create"
                    ...
                    (subscriber: NewsletterSubscriber, index: number) => (
                      <tr key={subscriber.id}>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <Link
                              href={`/newsletter-subscribers/edit/${subscriber.id}`}
                            ...
                            <button
                              onClick={() =>
                                handleDelete(subscriber.id, subscriber.phone)
                              }
