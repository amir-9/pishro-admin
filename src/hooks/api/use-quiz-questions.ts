...
// Query Keys
export const quizQuestionsKeys = {
  all: ['quiz-questions'] as const,
  lists: () => [...quizQuestionsKeys.all, 'list'] as const,
  list: (params?: { quizId?: string; page?: number; limit?: number }) => [...quizQuestionsKeys.lists(), params] as const,
  details: () => [...quizQuestionsKeys.all, 'detail'] as const,
  detail: (id: string) => [...quizQuestionsKeys.details(), id] as const,
};
...
export function useQuizQuestions(params?: { quizId?: string; page?: number; limit?: number }) {
  return useQuery({
    queryKey: quizQuestionsKeys.list(params),
    queryFn: async () => {
...
export function useQuizQuestion(id: string) {
  return useQuery({
    queryKey: quizQuestionsKeys.detail(id),
    queryFn: async () => {
...
export function useCreateQuizQuestion() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: quizQuestionsKeys.lists() });
    },
  });
}
  ...
  return useMutation({
    ...
    onSuccess: (response: QuizQuestionResponse) => {
      ...
      if (response.data && 'id' in response.data) {
        queryClient.invalidateQueries({ queryKey: quizQuestionsKeys.detail(response.data.id as string) });
      }
    },
  });
}
